...
public class BoardController extends UiUtils {
	...
	/*
	 * ModelAttribute : 파라미터로 전달받은 객체를 자동으로 뷰까지 전달할 수 있다. addAttribute(key,value)이런식으로 처리하지 않아도 되고 
	 * 				괄호안에 "params"가 별칭이므로 view 페이지에서는 별칭으로 접근하면 된다. 
	*/
	...
	public void downloadAttachFile(@RequestParam(value = "idx", required = false) final Long idx, Model model, HttpServletResponse response) {
		...
		Date insertDay = fileInfo.getInsertTime();
		SimpleDateFormat simple = new SimpleDateFormat("yyMMdd");
		String uploadDate = simple.format(insertDay);
